...
void inject_i2c_bus_handle(uint8_t port, void *handle)
{
    if (port < MAX_I2C_NUM && handle != NULL) {
        i2c_bus_handle[port] = (i2c_master_bus_handle_t)handle;
...
static int _i2c_init(uint8_t port)
{
    ...
    if (ret != ESP_OK) {
        // Bus already initialized (e.g. Arduino Wire) — reuse existing handle
        if (i2c_bus_handle[port] != NULL) {
            ESP_LOGI(TAG, "Reusing injected I2C bus handle on port %d", port);
            i2c_inited[port] = true;
            return 0;
        }
